...
void MenuState::changeScreen(std::shared_ptr<IState> &state, std::string s, CoreInfo &info, std::shared_ptr<UgandaEngine::AGameEngine> engine) {
    if (s == "MENU")
...
int    MenuState::exec() {
    return engine->_libGraph->handleMenu();
}
